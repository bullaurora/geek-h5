...
const Article = () => {
  ...
  // 获取文章详情
  // 注意：此处的第一个参数是 action，这个函数不能包含 async，所以，不管是直接给函数加 async
  //      还是在函数内部创建 async 函数，代码都会报错：Action 只能是 对象 或者通过 action creator 创建的 action
  //      也就是说：只能是同步，不能直接或间接使用 async
  ...
  //   load()
  // }, 'article')
  const getArticleByIdMemo = useCallback(()=>getArticleById(params.id||''),[params.id])
  const setLoadingMemo = useCallback(() => setLoading(false),[])
